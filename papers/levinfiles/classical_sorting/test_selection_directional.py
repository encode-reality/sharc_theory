"""
Test selection sort with directional comparison logic.

Hypothesis: The comparison direction should depend on whether
the cell is trying to move left or right.
"""
...
def selection_sort_directional(initial_values):
    """
    Selection sort where comparison depends on movement direction:
    - Moving left (idx > target): swap if cell.value < neighbor.value
    - Moving right (idx < target): swap if cell.value > neighbor.value
    """
    ...
    for timestep in range(MAX_STEPS):
        ...
        for idx in idx_order:
            ...
            should_swap = False
            if idx > target:
                # Moving left: swap if I'm smaller
                should_swap = (cell.value < neighbor.value)
            else:
                # Moving right: swap if I'm larger
                should_swap = (cell.value > neighbor.value)
...
# Test
print("=" * 70)
print("TESTING DIRECTIONAL COMPARISON LOGIC")
print("=" * 70)
...
for test_array in test_arrays:
    ...
    result, steps, history = selection_sort_directional(test_array.copy())
